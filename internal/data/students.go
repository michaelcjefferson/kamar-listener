...
func (m *StudentModel) InsertManyStudents(students []Student) error {
	...
	ON CONFLICT(uuid) DO UPDATE SET
		...
		firstnamelegal = excluded.firstnamelegal,
		lastname = excluded.lastname,
		lastnamelegal = excluded.lastnamelegal,
		forenames = excluded.forenames,
		forenameslegal = excluded.forenameslegal,
		gender = excluded.gender,
		genderpreferred = excluded.genderpreferred,
		gendercode = excluded.gendercode,
		schoolindex = excluded.schoolindex,
		email = excluded.email,
		mobile = excluded.mobile,
		house = excluded.house,
		whanau = excluded.whanau,
		boarder = excluded.boarder,
		byodinfo = excluded.byodinfo,
		ece = excluded.ece,
		esol = excluded.esol,
		ors = excluded.ors,
		languagespoken = excluded.languagespoken,
		datebirth = excluded.datebirth,
		startingdate = excluded.startingdate,
		startschooldate = excluded.startschooldate,
		leavingdate = excluded.leavingdate,
		leavingreason = excluded.leavingreason,
		leavingschool = excluded.leavingschool,
		leavingactivity = excluded.leavingactivity,
		moetype = excluded.moetype,
		ethnicityL1 = excluded.ethnicityL1,
		ethnicityL2 = excluded.ethnicityL2,
		ethnicity = excluded.ethnicity,
		iwi = excluded.iwi,
		yearlevel = excluded.yearlevel,
		fundinglevel = excluded.fundinglevel,
		tutor = excluded.tutor,
		timetablebottom1 = excluded.timetablebottom1,
		timetablebottom2 = excluded.timetablebottom2,
		timetablebottom3 = excluded.timetablebottom3,
		timetablebottom4 = excluded.timetablebottom4,
		timetabletop1 = excluded.timetabletop1,
		timetabletop2 = excluded.timetabletop2,
		timetabletop3 = excluded.timetabletop3,
		timetabletop4 = excluded.timetabletop4,
		maorilevel = excluded.maorilevel,
		pacificlanguage = excluded.pacificlanguage,
		pacificlevel = excluded.pacificlevel,
		siblinglink = excluded.siblinglink,
		photocopierid = excluded.photocopierid,
		signedagreement = excluded.signedagreement,
		accountdisabled = excluded.accountdisabled,
		networkaccess = excluded.networkaccess,
		altdescription = excluded.altdescription,
		althomedrive = excluded.althomedrive,
	...
	for i := 0; i < len(students); i += batchSize {
		...
		for _, s := range batch {
			...
			if len(s.LeavingSchool) > 0 {
				leavingSchoolJSON, err = json.Marshal(s.LeavingSchool)
				if err != nil {
					return fmt.Errorf("marshal leavingschool: %w", err)
				}
			} else {
